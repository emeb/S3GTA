...
esp_err_t eb_wm8731_write(uint8_t addr, uint16_t data)
{
	...
	/* send it */
    status = i2c_master_write_to_device(I2C_MASTER_NUM, W8731_ADDR_0, i2c_msg, sizeof(i2c_msg), pdMS_TO_TICKS(I2C_MASTER_TIMEOUT_MS));
 ...
 * set codec bypass
 */
void eb_wm8731_Bypass(uint8_t enable)
{
	/* set both chls volume */
	eb_wm8731_write(REG_APATH, 0x012 | enable ? 0x08 : 0x00);
}
