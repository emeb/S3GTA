...
void widg_button_render(btn_widg *btn, uint8_t state)
{
	if(btn->state != state)
	{
		...
		fore = gfx_get_forecolor();
		back = gfx_get_backcolor();
		
		btn->state = state;
	
		gfx_set_forecolor(state ? btn->on_color : btn->off_color);
		gfx_fillcircle(btn->x, btn->y, btn->r);
		gfx_set_backcolor(state ? btn->on_color : btn->off_color);
		gfx_set_forecolor(btn->text_color);
		gfx_drawstrctr(btn->x, btn->y-4, btn->text);
		
		gfx_set_forecolor(fore);
		gfx_set_backcolor(back);
