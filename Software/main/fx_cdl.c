/*
 * fx_cdl.c -  Clean Delay effect for esp32s2_audio
 * 03-14-22 E. Brombaugh
 */
...
void * fx_cd_common_Init(uint32_t *mem, uint8_t type)
{
	...
	/* set type / range */
	blk->type = type>>2;
	blk->rng = 3+(type&0x3)*2;
	blk->rng_raw = 0;
	
	/* init delay buffering */
	blk->dlybuf = fx_ext_buffer;
